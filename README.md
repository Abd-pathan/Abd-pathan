## 👋 Hi, I'm Abdullah Pathan

🚀 **Full-Stack Developer | Data Analyst | Cloud Enthusiast | SQL Developer**  
📍 Chicago, IL | 🌐 Open to full-time, contract & hybrid/remote roles  

I enjoy building helpful applications, analyzing data, and creating end-to-end solutions using modern tools.  
I work with **Java, Spring Boot, React, Python, SQL, AWS, Node.js** and love solving real-world problems through code.

---

## 🔧 Tech Stack

**Languages:** Java, JavaScript, Python, TypeScript, SQL  
**Frameworks:** Spring Boot, React, Node.js, Express, MERN stack  
**Databases:** PostgreSQL, MySQL, MongoDB, DynamoDB  
**Cloud & DevOps:** AWS, Docker, Git, GitHub Actions, GitLab CI/CD  
**Data Tools:** Pandas, NumPy, Scikit-Learn, Power BI, Tableau  

---

## 🚀 Featured Projects

### 🔹 Real-Time Chat Application  
📌 *React, Node.js, MongoDB, Socket.io*  
🔗 **Live Demo:** https://chat-application1-znm2.onrender.com/  
🔗 **Repo:** https://github.com/abd-pathan/Real-Time-Chat-Application  

Real-time messaging app with authentication, user presence indicators, and responsive UI.

---
### 🔹 Realtime Cricket Score Application 
📌 *Java, Spring Boot, Angular, Web Scraping, jsoup*
Created a real-time cricket score application providing live stats, including match scores and commentary.

### 🔹 E-commerce web application 
📌 *Java, Spring Boot, Angular, Docker, JWT Authentication*  
•	Developed a full-stack application integrating Angular for the frontend and Spring Boot for the backend, with Docker for containerization and deployment. 

### 🔹 Wine Quality Prediction  
📌 *Python, ML, Jupyter Notebook*  
•	Predicted wine quality using ML algorithms like Random Forest & Logistic Regression.

---


## 🏆 Certifications

- 🟦 **Microsoft Certified – Dynamics 365 Finance Functional Consultant**  
- ☁️ **AWS Certified Cloud Practitioner**

---

## 📫 Let's Connect  
📩 Email: abd.pathan812@gmail.com  
💼 LinkedIn: https://www.linkedin.com/in/abdullah-pathan  
🌐 Portfolio: https://abd-pathan.github.io/PORTFOLIO/

---
